      ...
      List.iter (fun (u, v, c) ->
        ...
        if 0 < Weight.compare inf d.(u) then
          ...
          if 0 < Weight.compare d.(v) dv then
            (is_modified := true; d.(v) <- if !i <= succ n lsr 1 then neg_inf else dv)) es;
        decr i
    done;
    Array.get d
end
